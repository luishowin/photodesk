...
v0.0, **Phase 0 complete**. Spec is at v0.17. All three spikes have run and two of the three answers were not the expected ones.
...
**Next in v0.1:** export — a rendered frame becomes a file with an ICC profile on it, which is where §12.3's source-preservation test finally has all five of its steps. Full-res export also wants tiling (§7.1), which the renderer does not do. Then the panels. §16 #17 (PNG, and therefore screenshots) is small and due before v0.1 ships; §16 #16 (parameter ranges) before v0.2's presets.
