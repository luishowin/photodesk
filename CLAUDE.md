...
v0.0, **Phase 0 complete**. Spec is at v0.18. All three spikes have run and two of the three answers were not the expected ones.
...
- **Generate binary fixtures; do not hand-write them.** Twice now a hand-built fixture for a format with an offset table has been wrong in a way that looked like a code bug — a JPEG rejected with "invalid length in DHT", and an EXIF block whose `ifd()` forgot the four-byte next-IFD pointer so Pillow read the camera make as `e iPho`. Generate it, check it with something that did not build it, and inline the bytes.
- **There is no CPU renderer and there must not be one.** The obvious way to test a GPU renderer is to write the same maths in Rust and compare — which is precisely the drift §0 freezes against. The shaders are the only description of what a pixel goes through. The two hardcoded constants in `adjust.wgsl` (linear P3's luma weights and its XYZ matrix) are the exception, and a test reads them out of the shader text and compares them against the derived values so they cannot drift.
- **§12.2's bar is a precondition, not a number.** Band-limited *and* in-gamut, or the measurement is about an inherent property rather than about the renderer. Don't loosen the threshold when it fails; check the fixture still satisfies both conditions first.
...
**The render path is done** (`engine/render.rs`, `shaders/photodesk/`, 8 tests). A compiled graph renders through wgpu. The product's shaders now live where §13 always said — `shaders/photodesk/` — and Spike C's `adjust.wgsl` stays a spike: it was built *hostile* to naga, and the product's implements the nine scalars `adjust` v1 actually carries, with stages 6–8 absent and keeping their numbers.

**§12.2 is measured, and it cost two wrong diagnoses.** With no spatial stages the expectation was near-exact agreement; the first run said 57 codes with *no adjustment at all*. It is not the chain (the identity control was just as bad) and it is not clipping (the disagreement sits away from the rails, not at them) — **it is the gamut map**, which is non-linear, so `f(mean)` ≠ `mean(f)`. The threshold therefore has to be stated against a precondition rather than as a number: band-limited **and** in-gamut gives **0.1486 codes**; detail finer than the proxy gives 152, of which 108 is stage 13 alone. The fixture asserts its own preconditions, so the bar cannot quietly stop being about the renderer.

**White balance is the slider that took work.** Temperature is a Kelvin delta applied as a von Kries scaling between two points on the CIE daylight locus, and two properties are tested rather than hoped for: its zero is *exactly* the identity (the gain is measured against the locus at the reference, not against D65's defined chromaticity), and it does not change brightness (0.17984 against 0.18 across ±2000 K).

**Export is done** (`engine/{exif,export}.rs`, an ICC writer, 12 tests), and **§12.3 runs end to end for the first time** — open, apply, render preview, export, close, and the source is byte-identical with the same mtime. Invariant #1 finally has all five of its steps.

Three things came out of it:

- **The tag we write is the one Apple writes.** lcms2 — which has never seen the writer — reads our Display P3 profile's red colorant as (0.5151, 0.2412, −0.0011), the same D50 triple §4 recorded off a real iPhone. Profiles are 444/456 bytes against Apple's 536. The creation date and profile ID are zeros on purpose: a timestamp would make every export of the same document a different file, which §12.1 could never bless.
- **Export found an orientation bug in the decoder.** The decode path did not read EXIF orientation at all, which is fine while the tag travels with the file and wrong the moment `metadata: strip` removes it — the export comes out rotated. Orientation is *structure*, not description. The decoder turns the pixels now (all eight cases, mirrors included) and the exporter writes 1.
- **`keep-minus-gps` removes rather than unreferences.** Deleting IFD0's pointer leaves the coordinates in the file for anything that walks the segment. The block is rebuilt from what survives: 262 bytes to 136. And IFD1's thumbnail is dropped under *every* policy — it is a picture of the source, so after a crop it hands back exactly what the crop removed.

**Next in v0.1:** the front end — everything above is headless, and §14's v0.1 is a photograph a person can open, drag six sliders on, compare against the original and export. That means Tauri (four devel packages are absent — see the environment note), the WebGL2 preview executing a compiled plan, and §11's slider contract hand-written. §16 #17 (PNG, and therefore screenshots) is small and due before v0.1 ships; #18 (tiled export) and #19 (HEIF output) can wait.
