...
v0.0, **Phase 0 complete**. Spec is at v0.14. All three spikes have run and two of the three answers were not the expected ones.
...
- **`src/document/generated/document.ts` is generated and committed.** Don't hand-edit it, and don't hand-write a second TypeScript description of the document — that is a frozen register item, and the reason is the same one that makes the browser harness load generated GLSL rather than a twin. `cargo test -p photodesk` regenerates it and fails if it changed, so the workflow is: change the Rust schema, run the tests once, commit both.
...
**Next in v0.1:** §6.2's graph compile — the document is stack-shaped on disk and compiles to a typed DAG, which is a build product and gets no UI ever. After that the decode path (`engine/`), then the six sliders, then export. §16 #16 (parameter ranges) is due before v0.2's presets, not before the graph.
